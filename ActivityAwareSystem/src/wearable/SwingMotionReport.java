...
public class SwingMotionReport {
	//int maxCluNum = 0;
	...
	public SwingMotionReport(String path, String iFilename, String oFilename, String o2Filename, int timewindow, int overlap) {
		// TODO Auto-generated constructor stub
		Path = path;
		IFileName = iFilename;
		OFileName = oFilename;
		O2FileName = o2Filename;
	...
	public void printReport(){
		try {
			...
			int cluNum = 0;
			for(int i=0; i<Statistic.get(0).size(); i++){
				int cluElementNum = 0;
				for(int j=0; j<Statistic.size();j++)
					cluElementNum += Statistic.get(j).get(i);
				if(cluElementNum!=0){
					cluNum++;
					fw.write( "c"+cluNum + ", " + cluElementNum + "\n");
					for(int j=0; j<L.size(); j++){
						double accumulateNum = (double)Statistic.get(j).get(i);
						double percent = Math.round((accumulateNum/(double)cluElementNum)*10000)/100.0;
						fw.write(formatStr(L.get(j), 11)+": "+ formatStr(percent, 6)+ "%,\t\t" + accumulateNum + "\n");
					}
					fw.write("\n\n");
				}
			}
	...
	public void generateHistogramFeature(){
			...
			try {
				...
				fw = new FileWriter(Path+"/Features/"+O2FileName);
	...
	private int findGroundTruth(int[] groundTruths){
		int max = 0;
		int secondMax = 0;
		int groundTruth = 0;
		int secondGroundTruth = 0;
		for(int j=0; j<groundTruths.length; j++){
			if(groundTruths[j]>max){
				groundTruth = j;
				max = groundTruths[j];
			}
			else if(groundTruths[j]>secondMax){
				secondGroundTruth = j;
				secondMax = groundTruths[j];
			}
		}
