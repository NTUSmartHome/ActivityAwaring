...
public class MeaningfulActReport {
	int maxCluNum = 0;
	...
	final int MAX = 1000;
	int existCluNum = 0;
	public MeaningfulActReport(String path, String ifilename, String ofilename) {
	...
	public void printExpReport(){
		try {
			...
			FileWriter fw2 = new FileWriter(Path+"/Report/"+"MeaningfulActionResultForExcel.csv");	
			int cluNum = 0;
			...
			for(int j=0; j<L.size(); j++){
				//int cluElementNum = CluElementNum.get(j);
				//if(cluElementNum!=0){
					for(int i=0; i<maxCluNum-1; i++){
					
						fw2.write(CLUSTER.get(j, i)+",");
					}
					fw2.write(CLUSTER.get(j, maxCluNum-1)+"");
					fw2.write("\n");
				//}
			}
